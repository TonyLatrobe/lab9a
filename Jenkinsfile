pipeline {
    ...
    environment {
        ...
        KUBECONFIG = "/tmp/microk8s-config.yaml"
    }
    ...
    stages {
        ...
        stage('Deploy OPA & App') {
            steps {
                sh '''
                    # Patch the real workspace path into both hostPath volumes
                    sudo microk8s kubectl apply -f k8s/opa.yaml
                    sudo microk8s kubectl patch deployment opa -n lab9 --type=json -p \
                      "[{\"op\":\"replace\",\"path\":\"/spec/template/spec/volumes/0/hostPath/path\",\"value\":\"$WORKSPACE/opa\"}]"

                    sudo microk8s kubectl apply -f k8s/app.yaml
                    sudo microk8s kubectl patch deployment lab9-app -n lab9 --type=json -p \
                      "[{\"op\":\"replace\",\"path\":\"/spec/template/spec/volumes/0/hostPath/path\",\"value\":\"$WORKSPACE/app\"}]"

                    sudo microk8s kubectl rollout status deployment/opa -n lab9 --timeout=90s
                    sudo microk8s kubectl rollout status deployment/lab9-app -n lab9 --timeout=90s
                    sudo microk8s kubectl get pods,svc -n lab9
                '''
            }
        }
        ...
        stage('Drift Detection') {
            steps {
                sh '''
                    ...
                    sudo microk8s kubectl patch resourcequota lab9-quota -n lab9 \
                      --type=merge \
                      -p '{"spec":{"hard":{"limits.cpu":"2000m"}}}'

                    echo ""
                    echo "==> Quota after manual patch (should show 2000m):"
                    sudo microk8s kubectl get resourcequota lab9-quota -n lab9 -o yaml | grep "limits.cpu"
            ...
            steps {
                sh '''
                    opa_test() {
                        ...
                        RESULT=$(sudo microk8s kubectl exec -n lab9 deploy/opa -- \
                          wget -qO- \
            ...
            steps {
                sh '''
                    app_test() {
                        ...
                        RESULT=$(sudo microk8s kubectl exec -n lab9 deploy/lab9-app -- \
        ...
        stage('Policy Update: Add charlie') {
            steps {
                sh '''
                    ...
                    sudo microk8s kubectl rollout restart deployment/opa -n lab9
                    sudo microk8s kubectl rollout status deployment/opa -n lab9 --timeout=60s
                    sleep 2
...
REGO
        ...
        stage('Teardown') {
            steps {
                sh '''
                    ...
                    sudo microk8s kubectl get namespace lab9 2>&1 || true

                    echo ""
                    echo "==> Deleting Jenkins (devops namespace):"
                    sudo microk8s kubectl delete namespace devops --ignore-not-found=true

                    echo ""
                    echo "==> Verifying devops namespace is gone:"
                    sudo microk8s kubectl get namespace devops 2>&1 || true
                '''
            }
        }
    }
    ...
    post {
        ...
        failure {
            sh 'sudo microk8s kubectl get pods -n lab9 || true'
            sh 'sudo microk8s kubectl get pods -n devops || true'
            sh 'sudo microk8s kubectl describe pods -n lab9 || true'
